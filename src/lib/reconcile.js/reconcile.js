(function (global, factory) {
    ...
    function generateMoves(map, nodes, indices, base, reverse, index) {
        ...
        // iterate over the CURRENT nodes and base nodes in the given order
        for (var i = 0, len = nodes.length; i < len; i++) {
                // current NODE
    ...
    function isEqualChange(change1, change2) {
        return change1['baseIndex'] === change2['baseIndex'] && change1['sourceIndex'] === change2['sourceIndex'] && change1['action'] === change2['action'] && change1['name'] === change2['name'] && change1['_deleted'] === change2['_deleted'] && change1['_inserted'] === change2['_inserted'] && change1['element'] && change2['element'] && change1['element'].nodeType === change2['element'].nodeType && (change1['element'].nodeType === 3 && change2['element'].nodeType === 3 && change1['element'].nodeValue === change2['element'].nodeValue || change1['element'].isEqualNode(change2['element'])) && change1['_textStart'] === change2['_textStart'] && change1['_textEnd'] === change2['_textEnd'];
    }
            ...
            for (; m < myLength; m++) {
                ...
                if (theirItem['baseIndex'] === myItem['baseIndex']) {
                    if (isEqualChange(theirItem, myItem)) {
                        ...
                        if ((theirItem['action'] === 'deleteText' || theirItem['action'] === 'insertText') && (myItem['action'] === 'deleteText' || myItem['action'] === 'insertText')) {
                    ...
                    conflicted = isParentChange(theirItem, myItem) && isMineRemove && !isTheirsRemove || isParentChange(myItem, theirItem) && !isMineRemove && isTheirsRemove;
                }
